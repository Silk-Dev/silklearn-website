        ...
        <MarketingHero
          ...
          rightChildren={
            <div className="grid gap-3 sm:grid-cols-2">
              {['Knowledge compilation', 'Canvas & synthesis', 'Team learning design', 'Structured reasoning'].map((item) => (
              ...
              {posts.map((post, index) => (
                <article key={post.slug} className={index > 0 ? 'border-t border-(--border) pt-6' : ''}>
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-[0.72rem] font-semibold uppercase tracking-[0.16em] text-(--muted-foreground)">
                      {post.eyebrow || 'Blog'} · {new Date(post.publishedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </p>
                  </div>
                  <TransitionLink className="mt-4 block text-[1.25rem] leading-tight tracking-[-0.02em] text-(--foreground)" href={`/blog/${post.slug}`}>
                    {post.title}
                  </TransitionLink>
